...
/*
QUERY BNF (syntax https://github.com/ofabricio/bnf)

	qry = WS* GROUP( ( '.'i qvl )+ ):qry
	qvl = str:str | arr:arr | idn:key | num | fun
	val = str:str | boo:boo | nil:nil | num | qry
	fun = GROUP('('i idn ')'i):fun
	arr = '['i GROUP(exp) WS* ']'i
	exp = trm WS* ROOT('|'):or  exp | trm | num | TEXT('*'):ast
	trm = fct WS* ROOT('&'):and trm | fct
	fct = qry WS* ROOT(cnd):cnd sub
	sub = WS* ( '('i exp WS* ')'i | val )
	cnd = '=='
	num = '\d+'r:num
	str = '"([^"\\\]|\\\.)*"'r
	idn = '[a-zA-Z_]+'r:idn
